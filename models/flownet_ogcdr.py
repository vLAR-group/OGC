...
from utils.flowstep3d_util import PointNetSetAbstraction, FlowEmbedding, PointNetFeaturePropogation


class Flow0Regressor(nn.Module):
    def __init__(self, npoint, use_instance_norm):
        super(Flow0Regressor, self).__init__()
        self.sa1 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=16, in_channel=64, mlp=[64, 64, 64],
...
class FlowRegressor(nn.Module):
    def __init__(self, npoint, use_instance_norm):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=16, in_channel=64, mlp=[64, 64, 64],
                                          group_all=False, use_instance_norm=use_instance_norm)
        self.sa2 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=16, in_channel=64, mlp=[64, 64, 64],
                                          group_all=False, use_instance_norm=use_instance_norm)
        self.fc = torch.nn.Linear(64, 3)
...
class GlobalCorrLayer(nn.Module):
    def __init__(self, npoint, use_instance_norm):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=int(npoint / 8), radius=None, nsample=8, in_channel=3, mlp=[32, 64, 64],
                                          group_all=False, use_instance_norm=use_instance_norm)
    ...
    def forward(self, pc1_l_glob, pc2_l_glob, feats1_glob, feats2_glob):
        pcloud1 = pc1_l_glob[2]
        pcloud2 = pc2_l_glob[2]
        ...
        flow0_us = self.fp0(pc1_l_glob[1], pc1_l_glob[2], None, flow0.permute(0, 2, 1).contiguous())
        _, corr_feats_l1 = self.sa1(pc1_l_glob[1], flow0_us)
        corr_feats = self.fp1(pc1_l_glob[0], pc1_l_glob[1], None, corr_feats_l1)
...
class EncoderLoc(nn.Module):
    def __init__(self, npoint, use_instance_norm):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=int(npoint / 2), radius=None, nsample=16, in_channel=3, mlp=[32, 32, 32],
                                          group_all=False, return_fps=True, use_instance_norm=use_instance_norm)
        self.sa2 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=16, in_channel=32, mlp=[64, 64, 64],
                                          group_all=False, return_fps=True, use_instance_norm=use_instance_norm)
...
class EncoderGlob(nn.Module):
    def __init__(self, npoint, use_instance_norm):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=int(npoint / 8), radius=None, nsample=16, in_channel=64, mlp=[128, 128, 128],
                                                 group_all=False, use_instance_norm=use_instance_norm)
        self.sa2 = PointNetSetAbstraction(npoint=int(npoint / 16), radius=None, nsample=8, in_channel=128, mlp=[128, 128, 128],
                                                 group_all=False, use_instance_norm=use_instance_norm)
...
class H0Net(nn.Module):
    def __init__(self, npoint, use_instance_norm):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=4, in_channel=64, mlp=[64, 64, 64],
                                        group_all=False, use_instance_norm=use_instance_norm)
        self.sa2 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=4, in_channel=64, mlp=[64],
...
class FlowStep3D(nn.Module):
    def __init__(self, npoint=2048, use_instance_norm=False, loc_flow_nn=8, loc_flow_rad=0.1, k_decay_fact=1.0):
        ...
        self.local_corr_layer = FlowEmbedding(radius=loc_flow_rad, nsample=loc_flow_nn, in_channel=64, mlp=[64, 64, 64],
                                              pooling='max', corr_func='concat', use_instance_norm=use_instance_norm)

        self.gru = GRU(npoint, hidden_dim=64, input_dim=64+64+16+3, use_instance_norm=use_instance_norm)

        self.flow_conv1 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=8, in_channel=3, mlp=[32, 32, 32],
                                          group_all=False, use_instance_norm=use_instance_norm)
        self.flow_conv2 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=4, in_channel=32, mlp=[16, 16, 16],
                                          group_all=False, use_instance_norm=use_instance_norm)
...
if __name__ == '__main__':
    ...
    npoint = 2048
    model = FlowStep3D(npoint).cuda()
    print(f'num_params:{sum(p.numel() for p in model.parameters() if p.requires_grad)}')
    pc1 = torch.randn(4, npoint, 3).cuda()
    pc2 = torch.randn(4, npoint, 3).cuda()

    flow_preds = model(pc1, pc2, pc1, pc2, iters=5)