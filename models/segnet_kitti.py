...
class MaskFormer3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.SA_modules.append(
            PointnetSAModuleMSG(
                npoint=int(n_point / 4), radii=[1, 2],
                nsamples=[64, 64], mlps=[[3, 32, 32, 32], [3, 32, 32, 64]], use_xyz=use_xyz,
                bn=bn
            )
        )
        self.SA_modules.append(
            PointnetSAModule(
                npoint=int(n_point / 8), radius=4,
                nsample=64, mlp=[32 + 64, 64, 64, 128], use_xyz=use_xyz,
                bn=bn
            )
        )
        self.SA_modules.append(
            PointnetSAModule(
                npoint=int(n_point / 16), radius=8,
                nsample=64, mlp=[128, 128, 128, 256], use_xyz=use_xyz,
                bn=bn
            )
        )
        self.FP_modules = nn.ModuleList()
        self.FP_modules.append(PointnetFPModule(mlp=[64 + 3, 64, 64, 64], bn=bn))
        self.FP_modules.append(PointnetFPModule(mlp=[32 + 64 + 128, 64, 64], bn=bn))
        self.FP_modules.append(PointnetFPModule(mlp=[128 + 256, 128, 128], bn=bn))
...
if __name__ == '__main__':
    ...
    pc = torch.randn(size=(4, 2048, 3)).cuda()
    point_feats = torch.randn(size=(4, 2048, 3)).cuda()
