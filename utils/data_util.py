...
def upsample_flow(pc, pc_fps, flow_fps):
    """
    Upsample per-point flows from a downsampled point set to the complete point cloud.
    :param pc: (1, N, 3) torch.Tensor.
    :param pc_fps: (1, N', 3) torch.Tensor.
    :param flow_fps: (1, N', 3) torch.Tensor.
    :return:
        flow: (1, N, 3) torch.Tensor.
    """
    ...
    flow_fps = flow_fps.transpose(1, 2).contiguous()
    flow = three_interpolate(flow_fps, nn_idx, weight)
...
def label_to_onehot(segms, max_n_object, ignore_npoint_thresh=0):
    """
