...
def load_segm(fpath):
    segm = Image.open(fpath)
    segm = np.array(segm)
    return segm


def filter_segm(segm, select_semantics=[]):
    valid_inst_id = 1
    segm_filtered = np.zeros(segm.shape[0], dtype=int)
    inst_ids = np.unique(segm)
    for inst_id in inst_ids:
        inst_semantic = inst_id // 256
        if inst_semantic in select_semantics:
            segm_filtered[segm == inst_id] = valid_inst_id
            valid_inst_id += 1
