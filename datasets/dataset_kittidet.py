...
from utils.data_util import compress_label_id, batch_segm_to_mask, batch_segm_to_mask_withconf, augment_transform
...
class KITTIDetectionDataset(Dataset):
    def __init__(self,
        ...
        :param load_prediction: path saving the predicted segmentation which will be loaded.
        :param load_confidence: whether load the confidence for each object (only for predicted segmentation).
        """
        ...
        if load_prediction is not None:
            self.predsegm_path = osp.join(data_root, 'segm_preds', load_prediction)
            print('Load segmentation predictions from', self.predsegm_path)
        else:
            self.predsegm_path = None
    ...
    def _load_predsegm(self, idx):
        data_path = osp.join(self.predsegm_path, self.data_ids[idx])
        segm = np.load(osp.join(data_path, 'segm.npy'))
        if self.load_confidence:
            conf = np.load(osp.join(data_path, 'conf.npy')).astype(np.float32)
        else:
            conf = np.ones((self.max_n_object), dtype=np.float32)
        return segm, conf
    ...
    def __getitem__(self, sid):
        ...
        if self.predsegm_path is not None:
            segm, conf = self._load_predsegm(sid)
        ...
        if self.onehot_label:
            ...
            if self.predsegm_path is not None:
                segms, valids = batch_segm_to_mask_withconf(segms, confs, self.max_n_object, self.ignore_npoint_thresh)
            else:
...
if __name__ == '__main__':
    ...
    data_root = '/home/ziyang/Desktop/Datasets/KITTI_Object'
    ...
    onehot_label = False
    max_n_object = 10
    dataset = KITTIDetectionDataset(data_root=data_root,
